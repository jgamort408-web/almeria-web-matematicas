...
\begin{frame}{¿Por qué organizar datos en una tabla?}
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{center}
      \begin{tikzpicture}
        \node[font=\Large, color=AlmTerra] (texto) at (0,0)
          {\faFileAlt};
        \node[font=\small, color=AlmTerra, below=2pt of texto]
          {Texto disperso};
        \draw[-Stealth, AlmAzulM, line width=2pt] (0.6,0) -- (1.6,0);
        \node[font=\Large, color=AlmVerde] (tabla) at (2.2,0)
          {\faTable};
        \node[font=\small, color=AlmVerde, below=2pt of tabla]
          {Tabla ordenada};
      \end{tikzpicture}
      \end{center}
    \end{column}
...
\begin{frame}{Estructura de una tabla de datos}
  ...
  \begin{tikzpicture}[font=\small, every node/.style={align=center}]
    ...
    \node[text=white, font=\bfseries\small] at (2.4,3.1)
      {Encabezado col.~1};
    \draw[AlmAzulM, line width=1pt] (4.8,0) -- (4.8,3.4);
    \node[text=white, font=\bfseries\small] at (6.4,3.1)
      {Encabezado col.~2};
    % Filas
    ...
    \draw[acota] (-0.7,2.8) -- (-0.7,3.4)
      node[midway, left=2pt, font=\tiny\bfseries, color=AlmAzulM]
      {\rotatebox{90}{Encabezados}};
    \draw[acota] (-0.7,0) -- (-0.7,2.8)
      node[midway, left=2pt, font=\tiny\bfseries, color=AlmGris]
      {\rotatebox{90}{Filas de datos}};
    \draw[<->, >=Stealth, AlmTerra, line width=0.8pt]
      (0,3.6) -- (4.7,3.6)
      node[midway, above, font=\tiny\bfseries, color=AlmTerra]
      {Columna \(x\) independiente};
    \draw[<->, >=Stealth, AlmVerde, line width=0.8pt]
      (4.9,3.6) -- (8,3.6)
      node[midway, above, font=\tiny\bfseries, color=AlmVerde]
...
\begin{frame}{Pasos para construir una tabla desde un texto}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{pasobox}[1]
        \small Lee con atención y \textbf{subraya todos los números}.
      \end{pasobox}
      \vspace{4pt}
      \begin{pasobox}[2]
        \small Identifica \textbf{qué representa cada número}
        (¿es una fecha? ¿una cantidad? ¿una medida?).
      \end{pasobox}
      \vspace{4pt}
      \begin{pasobox}[3]
        \small Decide cuál es la \textbf{variable independiente \(x\)}
        y cuál la \textbf{dependiente \(y\)}.
      \end{pasobox}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{pasobox}[4]
        \small Crea los \textbf{encabezados} con nombre claro y unidades
        entre paréntesis: \textit{Mes}, \textit{Visitantes (miles)}.
      \end{pasobox}
      \vspace{4pt}
      \begin{pasobox}[5]
        \small Rellena los valores en \textbf{orden creciente de \(x\)}
        (de menor a mayor).
      \end{pasobox}
      \vspace{4pt}
      \begin{pasobox}[6]
        \small \textbf{Verifica}: ¿están todos los datos?
...
\begin{frame}{Ejemplo 2: tarifa de taxi en Almería}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      ...
      \vspace{0.2cm}
      {\small $x$ = km recorridos \quad $y$ = precio total (€)}
    \end{column}
    \begin{column}{0.46\textwidth}
      \begin{center}
      \renewcommand{\arraystretch}{1.35}
...
\begin{frame}{De la tabla al gráfico: Alcazaba enero–mayo}
  ...
  \end{center}
  \vspace{-0.2cm}
  {\small\color{AlmGris} La tabla se convierte directamente en este gráfico
...
\begin{frame}{Evidencia del día}
  \begin{evidenciabox}
    ...
    \begin{tcolorbox}[colback=AlmAmbar!10, colframe=AlmAmbar!60!black,
                      arc=3pt, boxrule=0.8pt, fonttitle=\bfseries\small,
