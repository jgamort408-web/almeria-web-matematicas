    ...
    \begin{column}{0.42\textwidth}
      \begin{tikzpicture}[scale=0.92]
        ...
        \draw[recto, AlmAzulM] (A) -- (B)
          node[midway, below right, font=\tiny\bfseries, color=AlmAzulM]
          {distancia euclídea};
        % Ruta Manhattan
        \draw[rectot, dashed, line width=1.3pt]
          (A) -- (4.2,0.6) -- (B)
          node[midway, right, font=\tiny, color=AlmTerra]
          {ruta calles};
        % Acotaciones
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{tikzpicture}[scale=1.1]
        ...
        \draw[recto] (O) -- (A) node[midway, below, font=\small] {$a$};
        \draw[recto] (O) -- (B) node[midway, left, font=\small] {$b$};
        \draw[rectot, line width=2pt] (A) -- (B)
          node[midway, right, font=\small\bfseries, color=AlmTerra] {$c$};
        % Ángulo recto
        \draw[rectoang] (0.28,0) -- (0.28,0.28) -- (0,0.28);
        % Cuadrados visuales
        \fill[AlmAzulC, opacity=0.25]
          (0,-0.55) rectangle (3,-0.25) node[midway, font=\tiny, color=AlmAzul] {$a^2$};
        \fill[BRecordar, opacity=0.20]
          (-0.6,0) rectangle (-0.3,2.2) node[midway, font=\tiny, color=BRecordar, rotate=90] {$b^2$};
      \end{tikzpicture}
      \end{center}
      \vspace{4pt}
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.44\textwidth}
      \begin{tikzpicture}[scale=0.85]
        ...
        \node[punto] at (A) {};
        \node[punto] at (B) {};
        \node[etiqueta, below left, font=\small] at (A) {$A(1,1)$};
        \node[etiqueta, above left, font=\small] at (B) {$B(4,5)$};
        % Triángulo auxiliar
        \coordinate (C) at (4,1);
        ...
        \draw[rectot, line width=2pt] (A) -- (B);
        % Catetos
        \node[below, font=\tiny, color=AlmGris] at (2.5,1) {$\Delta x=3$};
        \node[right, font=\tiny, color=AlmGris] at (4,3) {$\Delta y=4$};
        \node[above left, font=\small\bfseries, color=AlmTerra] at (2.5,3)
          {$c=5$};
      \end{tikzpicture}
    \end{column}
  \end{columns}
    ...
    \begin{column}{0.52\textwidth}
      ...
      \begin{center}
        \tcbox[colback=AlmAmbar!15, colframe=AlmAmbar,
               fontupper=\bfseries\small, arc=4pt]{%
  ...
  \begin{columns}[c]
    \begin{column}{0.44\textwidth}
      \begin{tikzpicture}[scale=0.82]
        ...
        \node[puntov] at (Mer) {};
        \node[punto] at (Pto) {};
        \node[font=\tiny\bfseries, above right, color=AlmAzulM] at (Alc)
          {Alcazaba (1,4)};
        \node[font=\tiny\bfseries, above right, color=AlmTerra] at (Cat)
          {Catedral (2,2)};
        \node[font=\tiny\bfseries, above right, color=AlmVerde] at (Mer)
          {Mercado (3,2)};
        \node[font=\tiny\bfseries, below right, color=AlmAzulM] at (Pto)
          {Puerto (4,1)};
        % Ruta
