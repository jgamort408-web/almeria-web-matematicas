...
\begin{frame}{Tipos de variables: árbol de clasificación}
  \begin{center}
  \begin{tikzpicture}[
      every node/.style={font=\AlmFontSmall},
    ...
    % Hijos nivel 1
    \node[nodoc] (cual) at (-3.5,-1.6) {Cualitativa\\\AlmFontSmall(describe una\\cualidad)};
    \node[nodoc] (cuant) at (3.5,-1.6) {Cuantitativa\\\AlmFontSmall(valor numérico)};
    % Hijos nivel 2
    \node[nodod] (disc) at (2.0,-3.4) {Discreta\\\AlmFontSmall números enteros,\\\AlmFontSmall contables};
    \node[nodod] (cont) at (5.0,-3.4) {Continua\\\AlmFontSmall cualquier valor\\\AlmFontSmall en un intervalo};
    % Flechas
    ...
    % Ejemplos
    \node[font=\AlmFontTiny, text=AlmGris, align=center] at (-3.5,-2.9)
      {barrio, tipo de\\monumento, medio\\de transporte};
    \node[font=\AlmFontTiny, text=AlmGris, align=center] at (2.0,-4.5)
      {turistas/día,\\nº de autobuses};
    \node[font=\AlmFontTiny, text=AlmGris, align=center] at (5.0,-4.5)
...
\begin{frame}{Variable independiente y dependiente}
  ...
  \begin{tikzpicture}
    \node[rounded corners=5pt, fill=AlmAzulM!15, draw=AlmAzulM,
          minimum width=3.2cm, minimum height=1cm, font=\bfseries\AlmFontSmall,
          align=center, text width=3cm]
      (xi) at (0,0) {Mes del año\\\AlmFontSmall var.\ $x$};
    \node[rounded corners=5pt, fill=AlmAmbar!25, draw=AlmAmbar!70!black,
          minimum width=3.6cm, minimum height=1cm, font=\bfseries\AlmFontSmall,
          align=center, text width=3.4cm]
      (yi) at (7,0) {N.º de turistas\\\AlmFontSmall var.\ $y$};
    \draw[-Stealth, AlmTerra, line width=2pt] (xi.east) -- node[above,font=\AlmFontSmall,color=AlmTerra]
...
\begin{frame}{Turistas en Almería por mes: diagrama de puntos}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \node[font=\AlmFontTiny\bfseries, color=AlmTerra, above] at (axis cs:8,600)
        {Máximo: agosto};
      \node[font=\AlmFontTiny\bfseries, color=AlmAzulM, below right] at (axis cs:1,60)
        {Mínimo: enero};
    \end{axis}
  \end{tikzpicture}
  \end{center}
  \vspace{-0.1cm}
  {\AlmFontSmall $x$ = mes (variable independiente) \quad $y$ = turistas en miles (variable dependiente)}
\end{frame}
  ...
  \begin{errorbox}
    ...
    \begin{tikzpicture}
      \node[rounded corners=4pt, fill=AlmTerra!15, draw=AlmTerra,
            text width=5cm, align=center, font=\AlmFontSmall] (mal)
        at (0,0) {«El \textbf{número de turistas}\\
                  hace que \textbf{cambie el mes}»};
      \node[right=0.5cm of mal, font=\AlmFontLargeUp, color=AlmTerra] {\incorrecto};
    \end{tikzpicture}

    \vspace{0.4cm}
    \begin{tikzpicture}
      \node[rounded corners=4pt, fill=AlmVerde!12, draw=AlmVerde,
            text width=5cm, align=center, font=\AlmFontSmall] (bien)
        at (0,0) {«El \textbf{mes del año}\\
                  influye en el \textbf{número de turistas}»};
      \node[right=0.5cm of bien, font=\AlmFontLargeUp, color=AlmVerde] {\correcto};
    \end{tikzpicture}
    \end{center}
