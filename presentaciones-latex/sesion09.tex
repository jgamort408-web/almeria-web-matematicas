...
\begin{frame}{Perímetro vs. Área: tabla comparativa}
  ...
  \begin{tabular}{|>{\bfseries\color{AlmAzulM}}l|>{\centering\arraybackslash}p{3.8cm}|>{\centering\arraybackslash}p{3.8cm}|}
    ...
    \hline
    ¿Qué mide? &
      El \textbf{contorno}\\(la valla) &
      La \textbf{superficie}\\(el césped) \\
    \hline
    Dimensión &
      \textbf{1D} (lineal) &
  ...
  \begin{notabox}[Idea clave]
    \small Perímetro y área miden cosas \textbf{completamente distintas}.
  ...
  \begin{columns}[c]
    \begin{column}{0.62\textwidth}
      ...
      \begin{tikzpicture}[scale=0.78]
        ...
        \node[font=\scriptsize\bfseries, color=AlmTerra] at (2.4,0.8)
          {$P = 16$ \quad $A = 12$};
        ...
        \draw[acota] (0,-3.6) -- (3.2,-3.6)
          node[midway,below,font=\scriptsize]{4};
        \draw[acota] (3.55,-3.0) -- (3.55,-3.0+3.2)
          node[midway,right,font=\scriptsize]{4};
        \node[font=\scriptsize\bfseries, color=AlmVerde] at (1.6,-1.4)
          {$P = 16$ \quad $A = 16$};

        % Etiqueta
        \node[font=\scriptsize\bfseries, color=AlmAmbar!80!black]
          at (2.5,-0.1) {};
      \end{tikzpicture}
      \end{center}
    \end{column}
    \begin{column}{0.34\textwidth}
      \begin{teoriabox}
        ...
        un perímetro fijo.
        \\[4pt]
        \scriptsize(Problema isoperimérico)
      \end{teoriabox}
    \end{column}
  \end{columns}
\end{frame}
    ...
    \begin{column}{0.62\textwidth}
      ...
      \begin{tikzpicture}[scale=0.78]
        ...
        \draw[acota] (0,-0.55) -- (2.4,-0.55)
          node[midway,below,font=\scriptsize]{3};
        \draw[acota] (2.75,0) -- (2.75,3.2)
          node[midway,right,font=\scriptsize]{4};
        \node[font=\scriptsize\bfseries, color=AlmAzulM] at (1.2,1.6)
          {$P = 14$\quad$A = 12$};
        ...
        \draw[acota] (9.35,0.8) -- (9.35,2.4)
          node[midway,right,font=\scriptsize]{2};
        \node[font=\scriptsize\bfseries, color=AlmAmbar!80!black] at (6.6,1.6)
          {$P = 16$\quad$A = 12$};
      \end{tikzpicture}
      \end{center}
    \end{column}
...
\begin{frame}{¿Cuándo usar cada medida?}
  \begin{columns}[t]
    \begin{column}{0.48\textwidth}
      \begin{definicionbox}{\color{AlmTerra}Necesitas el PERÍMETRO}
        \small
        \begin{itemize}
    ...
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{definicionbox}{\color{AlmVerde}Necesitas el ÁREA}
        \small
        \begin{itemize}
  ...
  \begin{notabox}[Resumen]
    \small \textbf{Valla, bordillo, camino} $\rightarrow$ Perímetro (metros).
    \quad
...
\begin{frame}{Ejemplo completo: Rambla de Almería}
  \begin{columns}[c]
    \begin{column}{0.52\textwidth}
      \begin{ejemplobox}
        \small La \textbf{Rambla de Almería} es aproximadamente
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{tikzpicture}[scale=0.70]
        ...
        \draw[acota] (0,-0.5) -- (5.5,-0.5)
          node[midway,below,font=\tiny]{370 m};
        \draw[acota] (5.9,0) -- (5.9,0.75)
          node[midway,right,font=\tiny]{50 m};
        \node[font=\tiny\bfseries, color=AlmVerde!70!black] at (2.75,0.375)
          {Rambla de Almería};
        \node[font=\tiny\bfseries, color=AlmTerra] at (2.75,-0.95)
          {$P = 840$ m};
        \node[font=\tiny\bfseries, color=AlmVerde!70!black] at (2.75,-1.35)
          {$A = 18{.}500$ m$^2$};
      \end{tikzpicture}
      \end{center}
    \end{column}
...
\begin{frame}{Ejercicio 1 — RECORDAR \dificultad{1}}
  ...
  \begin{tikzpicture}[scale=0.80]
    ...
    \draw[acota] (0,-0.55) -- (4.8,-0.55)
      node[midway,below,font=\scriptsize]{12 m};
    \draw[acota] (5.15,0) -- (5.15,2.0)
      node[midway,right,font=\scriptsize]{5 m};
    \node[font=\small, color=AlmTerra] at (1.8,1.0) {$P = ?$\ m};
    \node[font=\small, color=AlmVerde] at (3.4,1.0) {$A = ?$\ m$^2$};
  \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}{Ejercicio 3 — APLICAR \dificultad{3}}
  \begin{aplicarbox}
    \small Un urbanista de Almería necesita:
    \begin{enumerate}
...
\begin{frame}{Tres rectángulos: misma P o misma A}
  ...
  \begin{tikzpicture}[scale=0.72]
    ...
    \node[above, font=\tiny\bfseries, color=AlmTerra] at (2.4,1.6)
      {$6 \times 2$};
    \node[font=\tiny, color=AlmTerra] at (2.4,0.8) {$P=16$};
    \node[font=\tiny, color=AlmTerra] at (2.4,0.3) {$A=12$};

    % --- Caso 2: 4x4, P=16, A=16 ---
    \fill[AlmVerde!15] (5.8,0) rectangle (5.8+3.2,3.2);
    \draw[rectov, line width=1.5pt] (5.8,0) rectangle (5.8+3.2,3.2);
    \node[above, font=\tiny\bfseries, color=AlmVerde] at (7.4,3.2)
      {$4 \times 4$};
    \node[font=\tiny, color=AlmVerde] at (7.4,1.8) {$P=16$};
    \node[font=\tiny, color=AlmVerde] at (7.4,1.2) {$A=16$};

    % Flecha entre 1 y 2
    \draw[->, AlmAmbar!80!black, line width=1.2pt]
      (5.0,0.8) -- (5.6,0.8);
    \node[above, font=\tiny\bfseries, color=AlmAmbar!80!black]
      at (5.3,0.8) {P igual};

    % --- Caso 3: 3x4, P=14, A=12 ---
    \fill[AlmAzulC!18] (0,-3.5) rectangle (2.4,-3.5+3.2);
    \draw[zona, line width=1.5pt] (0,-3.5) rectangle (2.4,-3.5+3.2);
    \node[below, font=\tiny\bfseries, color=AlmAzulM] at (1.2,-3.5)
      {$3 \times 4$};
    \node[font=\tiny, color=AlmAzulM] at (1.2,-2.0) {$P=14$};
    \node[font=\tiny, color=AlmAzulM] at (1.2,-2.5) {$A=12$};

    % Flecha entre 3 y 1 (misma A)
    \draw[->, AlmGris, dashed, line width=1.0pt]
      (2.6,-2.1) -- (4.4,-0.5);
    \node[right, font=\tiny\bfseries, color=AlmGris]
      at (3.5,-1.4) {A igual};
  \end{tikzpicture}
  \end{center}
  \begin{notabox}[Conclusión]
    \small Si cambias la forma de un rectángulo manteniendo el mismo
